...
class SetupConfig:
    ...
    skip_publications: bool = False  # Enable publication processing
...
class SystemInitializer:
    ...
    def __init__(self, config: Optional[SetupConfig] = None):
        ...
        self.config.max_workers = int(os.getenv('MAX_WORKERS', self.config.max_workers))
        self.config.batch_size = int(os.getenv('BATCH_SIZE', self.config.batch_size))
        self.config.checkpoint_hours = int(os.getenv('CHECKPOINT_HOURS', self.config.checkpoint_hours))
        
        logger.info("SystemInitializer initialized with config: %s", self.config.__dict__)
    ...
    async def initialize_web_content_processor(self) -> None:
        """Initialize the web content processor."""
        try:
            self.web_processor = WebContentProcessor(db=self.db, config=self.config.__dict__)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument('--batch-size', type=int, default=50, help='Batch size for web content processing')
